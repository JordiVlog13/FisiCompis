...
# Modificamos las posiciones para que el sistema sea geocentrico
x = np.zeros((cuerpos, len(posiciones[0])))
y = np.zeros_like(x)
if geocentrico:
    for j in range(len(posiciones[0])):
        for i in range(cuerpos):
            x[i,j] = posiciones[i].x[j] - posiciones[3].x[j]
            y[i,j] = posiciones[i].y[j] - posiciones[3].y[j]
else:
    for j in range(len(posiciones[0])):
        for i in range(cuerpos):
            x[i,j] = posiciones[i].x[j]
            y[i,j] = posiciones[i].y[j]
...
def update(frame):
    frame = factor * frame
    for i in range(cuerpos):
        planetas[i].set_data([x[i][frame]], [y[i][frame]])  # Usamos listas para que funcione
        trayectorias[i].set_data(x[i, :frame], y[i, :frame])
    return trayectorias + planetas
